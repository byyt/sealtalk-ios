...
//手机号码一行
@property(nonatomic, strong) UIView *phoneNumberBg;
...
- (void)phoneNumberLayout{
    //一整行
    _phoneNumberBg = [[UIView alloc] initWithFrame:CGRectZero];
    _phoneNumberBg.translatesAutoresizingMaskIntoConstraints = NO;
    _phoneNumberBg.userInteractionEnabled = YES;
    //    _phoneNumberBg.backgroundColor = [UIColor colorWithHexString:@"000000" alpha:0.0];
    [self.view addSubview:_phoneNumberBg];
    //加号
    ...
    [_phoneNumberBg addSubview:_quhaoIV];
    //86
    ...
    [_phoneNumberBg addSubview:_quhaoLabel];
    //手机号码输入区域
    _phoneNumberTextField = [[UITextField alloc] initWithFrame:CGRectZero];
    //    _phoneNumberTextField.backgroundColor = [UIColor clearColor];
    _phoneNumberTextField.tag = UserTextFieldTag;
    _phoneNumberTextField.delegate = self;
    //_account.placeholder=[NSString stringWithFormat:@"Email"];
    UIColor *color = [UIColor colorWithHexString:@"C5BABA" alpha:1.0];
    _phoneNumberTextField.attributedPlaceholder =
    [[NSAttributedString alloc] initWithString:@"手机号" attributes:@{NSForegroundColorAttributeName : color}];
    _phoneNumberTextField.textColor = [UIColor colorWithHexString:@"000000" alpha:1.0];
    //    _phoneNumberTextField.text = [self getDefaultUserName];
    _phoneNumberTextField.clearButtonMode = UITextFieldViewModeWhileEditing;
    _phoneNumberTextField.font = [UIFont fontWithName:@"Heiti SC" size:20.0];
    _phoneNumberTextField.keyboardType = UIKeyboardTypeNumberPad;
    _phoneNumberTextField.translatesAutoresizingMaskIntoConstraints = NO;
    [_phoneNumberBg addSubview:_phoneNumberTextField];
    ...
    [self.view addConstraint:[NSLayoutConstraint
                              ...
                              relatedBy:NSLayoutRelationEqual
                              toItem:_phoneNumberBg
                              attribute:NSLayoutAttributeCenterY
                              multiplier:1.0
                              constant:0]];
    
    NSDictionary *phoneNumberBgDic =
    NSDictionaryOfVariableBindings(_quhaoIV, _quhaoLabel, _phoneNumberTextField);
    NSArray *phoneNumberBgConstraints
    = [[[NSLayoutConstraint
         constraintsWithVisualFormat:
         @"V:|[_quhaoLabel]|" options:0 metrics:nil
         views:phoneNumberBgDic]
        arrayByAddingObjectsFromArray:[NSLayoutConstraint
                                       constraintsWithVisualFormat:
                                       @"V:|[_phoneNumberTextField]|" options:0 metrics:nil views:phoneNumberBgDic]]
       arrayByAddingObjectsFromArray:[NSLayoutConstraint
                                      constraintsWithVisualFormat:
                                      @"H:|[_quhaoIV]-7-[_quhaoLabel]-28-[_phoneNumberTextField(>=30)]|" options:0 metrics:nil views:phoneNumberBgDic]];
    
    [_phoneNumberBg addConstraints:phoneNumberBgConstraints];
}

//手机号码下方横线
- (void)phoneNumberLineLayout {
    _phoneNumberLine = [[UIView alloc] initWithFrame:CGRectZero];
    _phoneNumberLine.translatesAutoresizingMaskIntoConstraints = NO;
    _phoneNumberLine.backgroundColor = [UIColor colorWithHexString:@"979797" alpha:1.0];
    [self.view addSubview:_phoneNumberLine];
}
...
- (void)codeLayout{
    ...
    [_codeBg addSubview:_getVerificationCodeBt];
    
    //约束
    NSDictionary *views =
    NSDictionaryOfVariableBindings(_codeTextField, _getVerificationCodeBt);
    ...
    NSArray *getVerificationCodeBt_V= [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[_getVerificationCodeBt]|" options:0 metrics:nil views:views];
    NSArray *all_H= [NSLayoutConstraint constraintsWithVisualFormat:@"H:|[_codeTextField]-10-[_getVerificationCodeBt(95)]|" options:0 metrics:nil views:views];
    
    [_codeBg addConstraints:codeTextField_V];
    [_codeBg addConstraints:getVerificationCodeBt_V];
    [_codeBg addConstraints:all_H];
}
...
- (void)constraintAllView {
    ...
    //指定约束
    NSDictionary *views =
    NSDictionaryOfVariableBindings(_topLabel,_phoneNumberBg,_phoneNumberLine,_codeBg,_codeLine);
    
    
    NSArray *inputBackground_H= [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-55-[_phoneNumberBg]-55-|" options:0 metrics:nil views:views];
    NSArray *phoneNumberLine_H= [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-55-[_phoneNumberLine]-55-|" options:0 metrics:nil views:views];
    ...
    NSArray *all_V= [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-160-[_topLabel]-78-[_phoneNumberBg]-8-[_phoneNumberLine(1)]-30-[_codeBg]-8-[_codeLine(1)]" options:0 metrics:nil views:views];
    
    
    
    [self.view addConstraints:inputBackground_H];
    [self.view addConstraints:phoneNumberLine_H];
